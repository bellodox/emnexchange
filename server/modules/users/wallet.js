...
exports.onConfirmWithdraw = function(req, res)
{
    ...
    utils.GetSessionStatus(req, status => {
        ...
        if (info['blockWithdraw'] == true && status.id != 1)
            return utils.render(res, 'pages/user/wallet', {status: status, error: true, action: 'withdraw', message: '<b>Withdraw error:</b> Your withdrawals have been disabled. Please contact support.'});
